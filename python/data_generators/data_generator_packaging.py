...
OUTPUT_FILE = Path("packaging_telemetry.csv")
...
def get_operator_for_shift(packaging_operators, shift, shift_date):
    eligible = packaging_operators[packaging_operators["shift"] == shift]
...
def sensor_values(status, label_printer_temp, machine_wear, rng):
    if status == "RUNNING":
        units_packed_per_min = round(rng.uniform(8, 22), 2)
        barcode_scan_success_rate = round(rng.uniform(96.5, 99.9), 2)
        label_printer_temp += rng.uniform(0.00, 0.12)
        air_pressure_bar = round(rng.uniform(5.0, 7.5), 2)
        conveyor_speed_m_min = round(rng.uniform(4.0, 12.0), 2)
        power_kw = round(rng.uniform(4, 11), 2)

        if machine_wear > 75:
            barcode_scan_success_rate = round(rng.uniform(92.0, 97.5), 2)
            units_packed_per_min = round(units_packed_per_min * rng.uniform(0.75, 0.95), 2)

    elif status == "IDLE":
        units_packed_per_min = 0
        barcode_scan_success_rate = 0
        label_printer_temp -= rng.uniform(0.00, 0.08)
        air_pressure_bar = round(rng.uniform(3.0, 5.0), 2)
        conveyor_speed_m_min = 0
        power_kw = round(rng.uniform(1, 3), 2)

    elif status == "SETUP":
        units_packed_per_min = round(rng.uniform(1, 6), 2)
        barcode_scan_success_rate = round(rng.uniform(85.0, 96.0), 2)
        label_printer_temp += rng.uniform(-0.03, 0.08)
        air_pressure_bar = round(rng.uniform(4.0, 6.5), 2)
        conveyor_speed_m_min = round(rng.uniform(1.0, 4.0), 2)
        power_kw = round(rng.uniform(2, 6), 2)

    else:
        units_packed_per_min = 0
        barcode_scan_success_rate = 0
        label_printer_temp -= rng.uniform(0.02, 0.12)
        air_pressure_bar = 0
        conveyor_speed_m_min = 0
        power_kw = round(rng.uniform(0, 1), 2)

    label_printer_temp = round(max(20, min(65, label_printer_temp)), 2)
...
def generate_chunk(args):
    ...
    packaging_operators = operators_df[
        operators_df["assigned_station_id"] == WORK_STATION_ID
    ].copy()

    if packaging_operators.empty:
        raise ValueError(f"No operators found for station {WORK_STATION_ID}")

    temp_file = Path(f"packaging_telemetry_part_{chunk_id}.csv")
    ...
    label_printer_temp = round(rng.uniform(25, 34), 2)
    ...
    with temp_file.open("w", newline="", encoding="utf-8") as file:
        ...
        while current_time < chunk_end:
            ...
            (
                ...
                conveyor_speed_m_min,
                power_kw,
            ) = sensor_values(status, label_printer_temp, machine_wear, rng)
            ...
            telemetry_id = f"PK_01_TEL_{start_row_number + rows_written:07d}"
